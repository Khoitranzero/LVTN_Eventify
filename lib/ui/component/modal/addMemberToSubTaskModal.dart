...
class AddMemberToSubTaskSModal extends StatefulWidget {
  final String taskId;
  final String subTaskName;

  const AddMemberToSubTaskSModal({Key? key, required this.taskId, required this.subTaskName}): super(key: key);

  @override
  State<AddMemberToSubTaskSModal> createState() => _AddMemberToSubTaskSModalState();
}

class _AddMemberToSubTaskSModalState extends State<AddMemberToSubTaskSModal> {
  ...
  Future<void> fetchAllUsers() async {
    ...
    try {
      final response =
          await _taskRepository.getMemberAssignSubTask(eventProvider.taskId);
      if (response['EC'] == 0) {
        setState(() {
      ...
      for (final member in members) {
        if (member.isChecked) {
          final response = await _taskRepository.addMemberToTask(
              widget.taskId, member.userId);
          if (response['EC'] == 0) {
          await _notificationRepository.createNotifycation(
            'createuser',
            "Sự kiện ${eventProvider.eventName} : ${authProvider.fullName}, đã thêm ${member.name} vào công việc phụ ${widget.subTaskName}!",
            eventProvider.eventId,
            authProvider.id);
          } else {
              success = false;
            UIToastNN.showToastError(response['EM']);
            print('Failed to add user ${member.name} to the event: ${response['EM']}');
          }
        }
      }
       if (success) {
